...
class ChessList {
  static mode = 2; // 1 = P v P flip; 2 = Pw v Cb noflip; 3 = Pb v Cw noflip; 4 = C v C noflip
  static boardFlips = ChessList.mode;

  constructor() {
    this.turn = 2; // 2 is white, 1 is black
  ...
  drawIm() {
    ...
    for (let i = 0; i < this.thePiecesArray.length; i++) {
      if (
        this.thePiecesArray[i].isAlive == 1 &&
        this.thePiecesArray[i].type != 6
      ) {
    ...
    if (arr.length == 1) {
      return arr[0].type == 2 || arr[0].type == 3;
    }
    ...
    return (
      ...
      arr[0].type == 3 &&
      arr[1].type == 3 &&
      (arr[0].x + arr[0].y) % 2 == (arr[1].x + arr[1].y) % 2
    );
  }
  ...
  inCheck(color) {
    ...
    for (let i = 0; i < this.thePiecesArray.length; i++) {
      if (
        this.thePiecesArray[i].color == color &&
        this.thePiecesArray[i].type == 6
      ) {
  ...
  realMoveTo(i, x, y) {
    ...
    for (let z = 0; z < this.thePiecesArray.length; z++) {
      if (this.thePiecesArray[z].x == x && this.thePiecesArray[z].y == y) {
        this.thePiecesArray[z].isAlive = 0;
        this.drawCount = 0;
        this.movesList = [];
      }

      if (
        this.thePiecesArray[i].type == 1 &&
        this.thePiecesArray[i].x != x &&
        this.thePiecesArray[z].x == x &&
        this.thePiecesArray[i].y == this.thePiecesArray[z].y &&
        this.thePiecesArray[z].passant == 1
      ) {
        this.thePiecesArray[z].isAlive = 0;
        this.drawCount = 0;
        this.movesList = [];
    ...
    if (this.thePiecesArray[i].type == 1) {
      this.drawCount = 0;
    }

    if (
      this.thePiecesArray[i].type == 6 &&
      Math.abs(x - this.thePiecesArray[i].x) == 2
    ) {
      for (let z = 0; z < this.thePiecesArray.length; z++) {
        if (
          this.thePiecesArray[z].type == 4 &&
          this.thePiecesArray[z].color == this.thePiecesArray[i].color &&
          this.thePiecesArray[z].x > this.thePiecesArray[i].x ==
            x > this.thePiecesArray[i].x
        ) {
          this.realMoveTo(
            z,
            x +
              Math.abs(this.thePiecesArray[i].x - x) /
                (this.thePiecesArray[i].x - x),
            this.thePiecesArray[z].y,
            this.thePiecesArray
          );
        }
      }
    ...
    if (x == 1 && y == 1) {
      this.blackCastleLeft = 0;
    }
    if (x == 8 && y == 1) {
      this.blackCastleRight = 0;
    }
    if (x == 1 && y == 8) {
      this.whiteCastleLeft = 0;
    }
    if (x == 8 && y == 8) {
      this.whiteCastleRight = 0;
    }

    if (
      this.thePiecesArray[i].type == 4 &&
      this.thePiecesArray[i].x == 1 &&
      this.thePiecesArray[i].y == 1 &&
      this.thePiecesArray[i].color == 1
    ) {
      this.blackCastleLeft = 0;
    }
    if (
      this.thePiecesArray[i].type == 4 &&
      this.thePiecesArray[i].x == 8 &&
      this.thePiecesArray[i].y == 1 &&
      this.thePiecesArray[i].color == 1
    ) {
      this.blackCastleRight = 0;
    ...
    if (
      this.thePiecesArray[i].type == 4 &&
      this.thePiecesArray[i].x == 1 &&
      this.thePiecesArray[i].y == 8 &&
      this.thePiecesArray[i].color == 2
    ) {
      this.whiteCastleLeft = 0;
    }
    if (
    ...
    if (this.thePiecesArray[i].type == 6 && this.thePiecesArray[i].color == 2) {
      this.whiteCastleLeft = 0;
      this.whiteCastleRight = 0;
    }
    if (this.thePiecesArray[i].type == 6 && this.thePiecesArray[i].color == 1) {
      this.blackCastleLeft = 0;
      this.blackCastleRight = 0;
    }

    for (let j = 0; j < this.thePiecesArray.length; j++) {
      this.thePiecesArray[j].passant = 0;
    }

    if (
      this.thePiecesArray[i].type == 1 &&
      Math.abs(this.thePiecesArray[i].y - y) == 2
    ) {
      this.thePiecesArray[i].passant = 1;
    }
  ...
  canMoveTo(i, newX, newY) {
    ...
    if (this.canGoTo(i, newX, newY) == true) {
      for (let j = 0; j < this.thePiecesArray.length; j++) {
        if (
          this.thePiecesArray[j].x == newX &&
          this.thePiecesArray[j].y == newY &&
          this.thePiecesArray[j].isAlive == 1
        ) {
          this.thePiecesArray[j].isAlive = 0;
          ic = j;
        }

        if (
          this.thePiecesArray[i].type == 1 &&
          this.thePiecesArray[i].x != newX &&
          this.thePiecesArray[j].x == newX &&
          this.thePiecesArray[j].y == this.thePiecesArray[i].y &&
          this.thePiecesArray[j].passant == 1 &&
          this.thePiecesArray[j].isAlive == 1
        ) {
          this.thePiecesArray[j].isAlive = 0;
          ic = j;
      ...
      if (this.inCheck(this.thePiecesArray[i].color)) {
        this.moveTo(i, x, y);
        if (ic < 500) {
          this.thePiecesArray[ic].isAlive = 1;
        }
        return false;
      }
      this.moveTo(i, x, y);
  ...
  canGoTo(
    i,
    newX,
    newY // the big transition function
  ) {
    if (
      newX > 8 ||
      newX < 1 ||
      newY > 8 ||
      newY < 1 ||
      this.thePiecesArray[i].isAlive == 0
    ) {
      return false;
    }
    ...
    if (this.thePiecesArray[i].type == 1) {
      // pawn
      if (this.thePiecesArray[i].color == 1) {
        // black
        if (
          newX == this.thePiecesArray[i].x &&
          newY == this.thePiecesArray[i].y + 1
        ) {
          return (
            !this.pieceThere(newX, newY, 1) && !this.pieceThere(newX, newY, 2)
          );
        }

        if (
          this.thePiecesArray[i].y == 2 &&
          newY == this.thePiecesArray[i].y + 2 &&
          newX == this.thePiecesArray[i].x
        ) {
          if (
            this.pieceThere(
              this.thePiecesArray[i].x,
              this.thePiecesArray[i].y + 1,
              1
            ) ||
            this.pieceThere(
        ...
        if (
          Math.abs(newX - this.thePiecesArray[i].x) == 1 &&
          newY - this.thePiecesArray[i].y == 1
        ) {
          if (!this.pieceThere(newX, newY, 2)) {
            for (let j = 0; j < this.thePiecesArray.length; j++) {
              if (
                this.thePiecesArray[j].color == 2 &&
                this.thePiecesArray[j].passant == 1 &&
                this.thePiecesArray[j].x == newX &&
                this.thePiecesArray[j].y == this.thePiecesArray[i].y
              ) {
          ...
          return (
            this.pieceThere(newX, newY, 2) && !this.pieceThere(newX, newY, 1)
          );
        }
      }

      if (this.thePiecesArray[i].color == 2) {
        // white
        if (
          newX == this.thePiecesArray[i].x &&
          newY == this.thePiecesArray[i].y - 1
        ...
        if (
          Math.abs(newX - this.thePiecesArray[i].x) == 1 &&
          this.thePiecesArray[i].y - newY == 1
        ) {
          if (!this.pieceThere(newX, newY, 1)) {
            for (let j = 0; j < this.thePiecesArray.length; j++) {
              if (
                this.thePiecesArray[j].color == 1 &&
                this.thePiecesArray[j].passant == 1 &&
                this.thePiecesArray[j].x == newX &&
                this.thePiecesArray[j].y == this.thePiecesArray[i].y
              ) {
    ...
    if (this.thePiecesArray[i].type == 2) {
      // knight
      if (
        (Math.abs(newX - this.thePiecesArray[i].x) == 1 &&
          Math.abs(newY - this.thePiecesArray[i].y) == 2) ||
        (Math.abs(newX - this.thePiecesArray[i].x) == 2 &&
          Math.abs(newY - this.thePiecesArray[i].y) == 1)
      ) {
    ...
    if (this.thePiecesArray[i].type == 3) {
      // bishop
      if (
        !(
          Math.abs(newX - this.thePiecesArray[i].x) /
            Math.abs(newY - this.thePiecesArray[i].y) ==
          1
        )
      ) {
        return false;
      ...
      let numX =
        ...
        Math.abs(newX - this.thePiecesArray[i].x);
      let numY =
        (newY - this.thePiecesArray[i].y) /
        Math.abs(newY - this.thePiecesArray[i].y);

      for (let j = 1; j < Math.abs(newX - this.thePiecesArray[i].x); j++) {
        if (
          this.pieceThere(
            this.thePiecesArray[i].x + numX * j,
            this.thePiecesArray[i].y + numY * j,
            1
          )
        ) {
          return false;
    ...
    if (this.thePiecesArray[i].type == 4) {
      // rook
      if (
        !(
          (newX == this.thePiecesArray[i].x) ^
      ...
      for (
        ...
        if (newY == this.thePiecesArray[i].y) {
          ...
          if (
            this.pieceThere(
              this.thePiecesArray[i].x + num * j,
              this.thePiecesArray[i].y,
              1
            ) ||
            this.pieceThere(
              this.thePiecesArray[i].x + num * j,
    ...
    if (this.thePiecesArray[i].type == 5) {
      // queen
      if (
        !(
          (newX == this.thePiecesArray[i].x) ^
    ...
    if (this.thePiecesArray[i].type == 6) {
      // king
      if (
        newY == this.thePiecesArray[i].y &&
        Math.abs(newX - this.thePiecesArray[i].x) == 2
        ...
        if (
          this.thePiecesArray[i].color == 2 &&
          newX > this.thePiecesArray[i].x &&
          !this.pieceThere(6, 8, 1) &&
          !this.pieceThere(6, 8, 2) &&
          !this.pieceThere(7, 8, 1) &&
          !this.pieceThere(7, 8, 2)
        ) {
          return (
            this.whiteCastleRight == 1 &&
            !this.anyPieceGoTo(5, 8, this.thePiecesArray[i].color) &&
            !this.anyPieceGoTo(6, 8, this.thePiecesArray[i].color) &&
            !this.anyPieceGoTo(7, 8, this.thePiecesArray[i].color)
          );
        }
        ...
        if (
          this.thePiecesArray[i].color == 2 &&
          newX < this.thePiecesArray[i].x &&
          !this.pieceThere(4, 8, 1) &&
          !this.pieceThere(4, 8, 2) &&
          !this.pieceThere(3, 8, 1) &&
          !this.pieceThere(3, 8, 2) &&
          !this.pieceThere(2, 8, 1) &&
          !this.pieceThere(2, 8, 2)
        ) {
          return (
            this.whiteCastleLeft == 1 &&
            !this.anyPieceGoTo(5, 8, this.thePiecesArray[i].color) &&
            !this.anyPieceGoTo(4, 8, this.thePiecesArray[i].color) &&
            !this.anyPieceGoTo(3, 8, this.thePiecesArray[i].color)
          );
        }
        ...
        if (
          this.thePiecesArray[i].color == 1 &&
          newX > this.thePiecesArray[i].x &&
          !this.pieceThere(6, 1, 1) &&
          !this.pieceThere(6, 1, 2) &&
          !this.pieceThere(7, 1, 1) &&
          !this.pieceThere(7, 1, 2)
        ) {
          return (
            this.blackCastleRight == 1 &&
            !this.anyPieceGoTo(5, 1, this.thePiecesArray[i].color) &&
            !this.anyPieceGoTo(6, 1, this.thePiecesArray[i].color) &&
            !this.anyPieceGoTo(7, 1, this.thePiecesArray[i].color)
          );
        }
        ...
        if (
          this.thePiecesArray[i].color == 1 &&
          newX < this.thePiecesArray[i].x &&
          !this.pieceThere(4, 1, 1) &&
          !this.pieceThere(4, 1, 2) &&
          !this.pieceThere(3, 1, 1) &&
          !this.pieceThere(3, 1, 2) &&
          !this.pieceThere(2, 1, 1) &&
          !this.pieceThere(2, 1, 2)
        ) {
          return (
            this.blackCastleRight == 1 &&
            !this.anyPieceGoTo(5, 1, this.thePiecesArray[i].color) &&
            !this.anyPieceGoTo(4, 1, this.thePiecesArray[i].color) &&
            !this.anyPieceGoTo(3, 1, this.thePiecesArray[i].color)
          );
        }
      }
      ...
      return (
        ...
        Math.abs(newY - this.thePiecesArray[i].y) < 2 &&
        !(
          newX == this.thePiecesArray[i].x && newY == this.thePiecesArray[i].y
        ) &&
        !this.pieceThere(newX, newY, this.thePiecesArray[i].color)
      );
    }
  ...
  for (let x = 1; x < 9 && mainList.highlight != 0; x++) {
    for (let y = 1; y < 9; y++) {
      ...
      if (mouseIsOver(square, event)) {
        mainList.highlight2 = square.id;
        mouse = 1;
      } // if mouse hovers over a tile, set variable to its id
    }
  }
  if (mouse == 0) {
    mainList.highlight2 = -1;
  } // otherwise set variable to -1

  for (
    let i = 0;
    i < mainList.thePiecesArray.length &&
    mainList.highlight != 0 &&
    mainList.pause == 0;
    i++
  ) {
    ...
    if (
      mainList.thePiecesArray[i].x == x &&
      mainList.thePiecesArray[i].y == y
    ) {
      // if the chess piece is the one that came from highlighted tile (the one in hand)
...
for (let i = 0; i < mainList.thePiecesArray.length; i++) {
  ...
  function down(event) {
    if (mainList.game == 1 && mainList.pause == 0) {
      ...
      let squareId =
        mainList.thePiecesArray[i].x.toString() +
        "$" +
        mainList.thePiecesArray[i].y.toString();
      mainList.highlight = squareId; // sets the highlight to the id of the tile object
      document.onmouseup = function () {
        if (mainList.highlight == squareId) {
          mainList.highlight = 0;
          mainList.highlight2 = -1;
          tryToMove(i, mainList);
        }
      };
    }
  }
...
function myFunction(mainList) {
  ...
  if (mainList.inRealStalemate(mainList.turn)) {
    ...
    setInterval(drawBoard, 1, mainList);
    setTimeout(function () {
      alert("Game over due to stalemate.");
    }, 100);
  }
  ...
  if (mainList.drawIm()) {
    ...
    setInterval(drawBoard, 1, mainList);
    setTimeout(function () {
      alert("Game over due to insufficient mating material.");
    }, 100);
  }
  ...
  if (mainList.drawCount > 99) {
    ...
    setTimeout(function () {
      alert(
        "Game over because no piece was captured and no pawn moved in the previous 50 moves."
      );
    }, 100);
  }
  ...
  for (let i = 1; i < mainList.movesList.length; i += 2) {
    if (mainList.movesList[i] > 2) {
      ...
      setInterval(drawBoard, 1, mainList);
      setTimeout(function () {
        alert("Game over due to repetition.");
      }, 100);
    }
  }
  ...
  for (let x = 1; x < 9; x++) {
    for (let y = 1; y < 9; y++) {
      ...
      if (mouseIsOver(square, event)) {
        if (
          (ChessList.mode == 1 ||
            (ChessList.mode == 2 && mainList.turn == 2) ||
            (ChessList.mode == 3 && mainList.turn == 1)) &&
          ...
          while (
            counter == 1 &&
            mainList.thePiecesArray[i].type == 1 &&
            ((mainList.thePiecesArray[i].y == 8 &&
              mainList.thePiecesArray[i].color == 1) ||
              (mainList.thePiecesArray[i].y == 1 &&
                mainList.thePiecesArray[i].color == 2))
          ) {
            let prom = prompt("What to promote to?");
            if (prom == "knight") {
              counter = 0;
              mainList.promotePawn(i, 2);
            }
            if (prom == "bishop") {
              counter = 0;
              mainList.promotePawn(i, 3);
            }
            if (prom == "rook") {
              counter = 0;
              mainList.promotePawn(i, 4);
            }
            if (prom == "queen") {
              counter = 0;
              mainList.promotePawn(i, 5);
            }
          }
          ...
          setTimeout(function () {
            ...
            if (
              ChessList.mode == 4 ||
              (mainList.turn == 2 && ChessList.mode == 3) ||
              (mainList.turn == 1 && ChessList.mode == 2)
            ) {
...
function computerMove(mainList) {
  ...
  mainList.realMoveTo(ii, xx, yy);

  if (
    mainList.thePiecesArray[ii].type == 1 &&
    ((mainList.thePiecesArray[ii].y == 8 &&
      mainList.thePiecesArray[ii].color == 1) ||
      (mainList.thePiecesArray[ii].y == 1 &&
        mainList.thePiecesArray[ii].color == 2))
  ) {
    mainList.promotePawn(ii, 5);
  }
...
function editMove(piecesList) {
  ...
  for (let t = 0; t < piecesList.thePiecesArray.length; t += 1) {
    piecesList.movesList[piecesList.movesList.length - 1][c] =
      piecesList.thePiecesArray[t].x;
    piecesList.movesList[piecesList.movesList.length - 1][c + 1] =
      piecesList.thePiecesArray[t].y;
    piecesList.movesList[piecesList.movesList.length - 1][c + 2] =
      piecesList.thePiecesArray[t].type;
    piecesList.movesList[piecesList.movesList.length - 1][c + 3] =
      piecesList.thePiecesArray[t].isAlive;
    piecesList.movesList[piecesList.movesList.length - 1][c + 4] =
      piecesList.thePiecesArray[t].passant;
    c += 5;
  }

  piecesList.movesList[piecesList.movesList.length - 1][c] =
    piecesList.blackCastleRight;
  piecesList.movesList[piecesList.movesList.length - 1][c + 1] =
    piecesList.blackCastleLeft;
  piecesList.movesList[piecesList.movesList.length - 1][c + 2] =
    piecesList.whiteCastleRight;
  piecesList.movesList[piecesList.movesList.length - 1][c + 3] =
    piecesList.whiteCastleLeft;
  ...
  for (let t = 0; t < piecesList.movesList.length - 2; t += 2) {
    if (
      arrEquals(
        piecesList.movesList[t],
        piecesList.movesList[piecesList.movesList.length - 2]
      )
    ) {
      piecesList.movesList[t + 1]++;
    }
  }
...
function drawBoard(piecesList) {
  ...
  if (piecesList.inCheck(2)) {
    document.getElementById("1").style.textShadow =
      "1px 1px 1.5px #Bf3232, -1px -1px 1.5px #Bf3232, -1px 1px 1.5px #Bf3232, 1px -1px 1.5px #Bf3232";
  } else {
    document.getElementById("1").style.textShadow = "";
  }

  if (piecesList.inCheck(1)) {
    document.getElementById("0").style.textShadow =
      "1px 1px 1.5px #Bf3232, -1px -1px 1.5px #Bf3232, -1px 1px 1.5px #Bf3232, 1px -1px 1.5px #Bf3232";
  } else {
    document.getElementById("0").style.textShadow = "";
  }
  ...
  for (let i = 0; i < arr1.length; i++) {
    ...
    if (ChessList.boardFlips == 1) {
      if (piecesList.turn == 1) {
        xPosit = 7 - xPosit;
        yPosit = 7 - yPosit;
      }
      if (piecesList.pause == 1) {
        xPosit = 7 - xPosit;
        yPosit = 7 - yPosit;
      }
    }
  ...
  for (let xPos = 1; xPos < 9; xPos++) {
    for (let yPos = 1; yPos < 9; yPos++) {
      ...
      if (piecesList.red != square.id) {
        if ((xPos + yPos) % 2 == 1) {
          square.style.backgroundColor = "#ab8a68";
        } else {
          square.style.backgroundColor = "#e6cdb4";
        }
      }
    }
  }
  ...
  if (piecesList.highlight != 0) {
    document.getElementById(piecesList.highlight).style.backgroundColor =
      "#967694";
    if (piecesList.highlight2 != -1) {
      document.getElementById(piecesList.highlight2).style.backgroundColor =
        "#967694";
    }
  }
  ...
  for (
    let i = 0;
    i < piecesList.thePiecesArray.length && piecesList.highlight == 0;
    i++
  ) {
    ...
    if (
      piecesList.thePiecesArray[i].type == 2 &&
      piecesList.thePiecesArray[i].color == 2
    ) {
      document.getElementById(i).innerHTML = "&#9816";
    }
    if (
      piecesList.thePiecesArray[i].type == 3 &&
      piecesList.thePiecesArray[i].color == 2
    ) {
      document.getElementById(i).innerHTML = "&#9815";
    }
    if (
      piecesList.thePiecesArray[i].type == 4 &&
      piecesList.thePiecesArray[i].color == 2
    ) {
      document.getElementById(i).innerHTML = "&#9814";
    }
    if (
      piecesList.thePiecesArray[i].type == 5 &&
      piecesList.thePiecesArray[i].color == 2
    ) {
      document.getElementById(i).innerHTML = "&#9813";
    }

    if (
      piecesList.thePiecesArray[i].type == 2 &&
      piecesList.thePiecesArray[i].color == 1
    ) {
      document.getElementById(i).innerHTML = "&#9822";
    }
    if (
      piecesList.thePiecesArray[i].type == 3 &&
      piecesList.thePiecesArray[i].color == 1
    ) {
      document.getElementById(i).innerHTML = "&#9821";
    }
    if (
      piecesList.thePiecesArray[i].type == 4 &&
      piecesList.thePiecesArray[i].color == 1
    ) {
      document.getElementById(i).innerHTML = "&#9820";
    }
    if (
      piecesList.thePiecesArray[i].type == 5 &&
      piecesList.thePiecesArray[i].color == 1
    ) {
      document.getElementById(i).innerHTML = "&#9819";
    }
